...
def get_players(db:Session, skip: int=0, limit: int = 100,
    ...
    if first_name:
        query.filter(models.Player.first_name == first_name)
    if last_name:
        query.filter(models.Player.last_name == last_name)  
...
def get_teams(db:Session, skip:int = 0, limit:int = 100, 
              min_last_changed_date:date = None, team_name:str = None, league_id: int =None ):
